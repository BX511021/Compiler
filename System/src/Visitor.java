...
//        写入符号表
    ...
    private void ConstDecl(Node syn){
        ...
        while (ckLfN(export(cs, i), SYMBOL.COMMA)) {//检测逗号，向后循环读入
            i++;
            ConstDef(export(cs, i++), type);
        }
    ...
    private void ConstDef(Node syn, BasicType type){
        ...
        /*节点测试结束
            开始进行纬度测试，在符号表中创建表象，错误处理的情况下，创建未知进行占位
        */
        ...
        if (dimensions.size() == 0) {
            ...
            try {
                ...
                FindError(ErrorType.duplicatedDefinitionError(((LeafNode) export(cs, mark)).getToken().getLine()));
            }
...
//                    检测returnTTK
            ...
            if (!lastElementReturnExp) {
                FindError(ErrorType.intFuncButNoReturnError(lineAtLastBrace));
            }
        }
    ...
    private void MainFuncDef(Node syn){
        ...
        try {
            ...
            funcHead = symbolTable.getPresentBlock().createUnknownFunc();
            FindError(ErrorType.duplicatedDefinitionError(((LeafNode) export(cs, i++)).getToken().getLine()));
        }
...
//        }
    ...
    private void Stmt(Node syn){
        ...
        if (ckLfN(export(cs, i), SYMBOL.IFTK)) {
            ...
            if (!symbolTable.getPresentBlock().isInWhileBlock()) {
                FindError(ErrorType.breakContinueError(((LeafNode) export(cs, i)).getToken().getLine()));
            }
            ...
            if (ckBrN(export(cs, i), GUNIT.Exp)) {
                ...
                if (lastFunc != null && lastFunc.funcHead.dataType.equals(BasicType.VOID)) {
                    FindError(ErrorType.voidFuncButReturnError(((LeafNode) export(cs, mark)).getToken().getLine()));
                }
                i++;
            } else {
            ...
            for (int iter = 1; iter < size - 1; iter++) {   // 去除头尾的引号
                if (!((strArray[iter] == 32) || (strArray[iter] == 33) || (strArray[iter] <= 126 && strArray[iter] >= 40))) {
                    if (!(strArray[iter] == '%' && strArray[iter + 1] == 'd')) {
                        FindError(ErrorType.formatStringError(((LeafNode) export(cs, i)).getToken().getLine()));
                        break;
                    }
                }
            ...
            if (countD != countE) {
                FindError(ErrorType.printfExpAmountError(((LeafNode) export(cs, i)).getToken().getLine()));
            }
            ...
            if (toModify instanceof VarSymbolItem && ((VarSymbolItem) toModify).isConst) {
                FindError(ErrorType.modifiedConstError(((LeafNode) (export(cs, i).getChildren().get(0))).getToken().getLine()));
    ...
    private SymbolItem LVal(Node syn){
        ...
        if (found == null) {
            FindError(ErrorType.undefinedError(((LeafNode) export(cs, mark)).getToken().getLine()));
            return symbolTable.getPresentBlock().createUnknown();
        }
        ...
        if (found instanceof VarSymbolItem) {
            ...
            ErrorType.warning("unexpected SymbolItem found in LVal");
            return symbolTable.getPresentBlock().createUnknown();
        }
    }
    ...
    private SymbolItem UnaryExp(Node syn){
        ...
        if (ckBrN(export(cs, i), GUNIT.PrimaryExp)) {
            ...
            if (found == null) {
                FindError(ErrorType.undefinedError(((LeafNode) export(cs, i)).getToken().getLine()));
                return symbolTable.getPresentBlock().createUnknown();
            }
            ...
            if (found != null) {
                ...
                if (countF != countR) {
                    FindError(ErrorType.parameterAmountError(((LeafNode) funcNameNode).getToken().getLine()));
                } else {
                    for (int iter = 0; iter < countF; iter++) {
                        ...
                        if (!fpureType.equals(rpureType)) {
                            FindError(ErrorType.parameterTypeError(((LeafNode) funcNameNode).getToken().getLine()));
                            return symbolTable.getPresentBlock().createUnknown();
                        }
                    }
                }
...
//            }
        ...
        ErrorType.warning("UnaryExp not detected");
        return null;
    }
    ...
    private Integer calNode(Node syn){
            ...
            switch (type) {
                case IDENFR:
                    ...
                    try {
                        ...
                        ErrorType.warning("when try to cal value from a ConstExp, we encounter a var can't be transformed to VarSymbolItem");
                    }
            ...
            switch (type) {
                ...
                case UnaryExp:
                    ...
                    if (ckLfN(uchildren.get(0), SYMBOL.IDENFR)) {
                        ErrorType.warning("a func is detected while calculating ConstExp");
                        ...
                        if (unaryOp.equals(SYMBOL.PLUS)) {
                            ...
                            ErrorType.warning("a ! is detected when calculating ConstExp");
                        }
                    }
                case PrimaryExp:
        ...
        ErrorType.warning("while calculating ConstExp, we encounter a unexpected type of node");
        return null;
    }
