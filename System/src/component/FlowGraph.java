...
import symboltable.*;
import Node.Label;
import component.optimizers.ConstOptimizer;
import component.optimizers.GlobalExpOptimizer;
import component.optimizers.SeekOptimizer;
import component.regpool.RegPool;
import global.Errorpart.Config;
...
//            Logger.GetLogger().IMExpLogPath("// " + blk.name + " begins //");
            System.out.println("// " + blk.name + " begins //");
...
//            Logger.GetLogger().IMExpLogPath("// " + blk.name + " ends //");
            System.out.println("// " + blk.name + " ends //");
        }
        ...
        for (int i = blocks.size() - 1; i >= 0; i--) {
            ...
            while (queue.size() > 0) {
                ...
                if (!setCmp(head.activeIn, temp2)) {
                    ...
                    if (check1.size() > 0) {
                        ErrorType.warning("BasicBlock.activeIn is Shrinking, which is not expected");
                    }
                    head.activeIn = temp2;
