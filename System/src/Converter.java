...
import Node.*;
import convert.imcode.imexp.IMExpType;
import convert.imcode.imexp.IMFac;
import convert.narray.*;
import datatype.ArrayType;
import datatype.Datatype;
import datatype.IntType;
import datatype.VoidType;
import symbolstruct.CodeRegion;
import symbolstruct.FuncRegion;
import symbolstruct.GlobalRegion;
import symbolstruct.Symer;
...
public class Converter {
    ...
    private String genTmp() {
        return "_TEMP_num_" + this.midVarCounter++;
    }

    private String genStrMarker() {
        return "_STR_VALUE_num_" + this.midVarCounter++;
    }
        ...
        if (dimensions.size() == 0) {
            ...
            for (int offset = 0; offset < arrayType.flat(); offset++) {
                ...
                if (instance instanceof Integer) {
                    ...
                    ErrorType.warning("We find non-integer item in const-array-initial when we need to assign to pos");
                }
            }
        }
    }
            ...
            if (value != null) {
                for (int offset = 0; offset < arrayType.flat(); offset++) {
                    ...
                    if (instance instanceof VarEntry) {
                        ...
                        valueVar = null;
                        ErrorType.warning("Unknown type of object extracted from array when we need to assignToPos");
                    }
        ...
        if (ckLfN(export(cs, i), SYMBOL.LBRACE)) {
            ...
            if (!ckLfN(export(cs, i), SYMBOL.RBRACE)) {
                ...
                if (child instanceof ConstNArray) {
                    ...
                    ErrorType.warning("unknown type of child returns in ConstInitialVal");
                }
            ...
            if (((ArrayType) reference).dimension.size() != 1 && itemList.size() != referenceLen) {
                ...
                ErrorType.warning("Initial an array but lost some dimensions, it's not allowed when this is high dimension");
...
//            codeRegion.addIMExp(IMFac.gen(IMExpType.ConNotJump, while_end_label, cond));
            ...
            if (entry instanceof VarEntry) {
                ...
                if (lvalNode.getChildren().size() > 3 &&
                    ...
                    if (npos.getDims() != ((ArrayType) arrayEntry.datatype).dimension.size()) {
                        ErrorType.warning("LVal can't be an array when it's a left-value");
                    }
                    ...
                    ErrorType.warning("LVal is an array but with no [], this is not allowed");
                }
            }
    ...
    private void LOrExp(Node syn, Label yesLabel, Label failLabel, Boolean isYesFollow) {
        ...
        if (ckBrN(export(cs, i), GUNIT.LAndExp)) {
            ...
            if (op.equals("||")) {
                ...
                ErrorType.warning("Unsupported operator in LOrExp");
            }
        }
    }
    ...
    private Entry AddExp(Node syn) {
        ...
        if (ckBrN(export(cs, i), GUNIT.MulExp)) {
            ...
            if (op.equals("+")) {
                ...
                ErrorType.warning("Unsupported operator in AddExp");
                return null;
            }
        }
    }
    ...
    private void LAndExp(Node syn, Label yesLabel, Label failLabel, Boolean isYesFollow) {
        ...
        if (ckBrN(export(cs, i), GUNIT.EqExp)) {
            ...
            if (op.equals("&&")) {
                ...
                ErrorType.warning("Unsupported operator in LAndExp");
            }
        }
    }
    ...
    private Entry MulExp(Node syn) {
        ...
        if (ckBrN(export(cs, i), GUNIT.UnaryExp)) {
            ...
            if (op.equals("*")) {
                ...
                ErrorType.warning("Unsupported operator in MulExp");
                return null;
            }
        }
    }
    ...
    private Entry EqExp(Node syn) {
        ...
        if (ckBrN(export(cs, i), GUNIT.RelExp)) {
            ...
            if (op.equals("==")) {
                ...
                ErrorType.warning("Unsupported operator in EqExp");
                return null;
            }
        }
    }
    ...
    private Entry RelExp(Node syn) {
        ...
        if (ckBrN(export(cs, i), GUNIT.AddExp)) {
            ...
            if (op.equals(">")) {
                ...
                ErrorType.warning("Unsupported operator in RelExp");
                return null;
            }
        }
    }
    ...
    private Entry LVal(Boolean isLeft, Node syn) {
        ...
        if (isLeft) {
            ErrorType.warning("When LVal plats the left-value role, it should be processed in Stmt instead of LVal");
            return null;
        } else {
            if (entry instanceof VarEntry) {
